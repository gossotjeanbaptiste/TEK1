...
void adder(sfVector2f ***points, sfVertexArray *vertexArray, map_t *map)
{
    ...
    for (int y = 0; y < 11; y++) {
        for (int x = 0; x < 11; x++) {
            if (points[y][x] && points[y][x + 1] && points[y + 1][x] && points[y + 1][x + 1]) {
                ...
                adder_condition(y, x, vertexArray, point, map);
            }
        }
    }
    free(point);
