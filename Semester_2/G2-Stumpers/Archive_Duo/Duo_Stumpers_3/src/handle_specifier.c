...
static int verify_characters(int index, int ac, char const *const *av)
{
    if (index + 1 >= ac)
        return 0;
    if (index + 1 < ac) {
...
static int verify_number(int index, int ac, char const * const *av)
{
    if (index + 1 >= ac)
        return 0;
    if (index + 1 < ac) {
...
void handle_specifier(int ac, char const * const *av, flag_t *flags)
{
    flags->flag_n = NULL;
    flags->flag_s = NULL;
    for (int i = 1; i < ac; i++) {
